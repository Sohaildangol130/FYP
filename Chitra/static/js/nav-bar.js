...
const display_checkout_items = () => {
    $.ajax({
        ...
        success: (data) => {
            if (items.length != 0 ){
                $('.checkout-box__container').show();
                $('.check-box__container--empty').hide();
                ...
                $.each(data, (key, value)=>{
                    value.forEach(element => {
                        $('.nav-item--checkout-box__container__all-checkout-items').prepend(
                            `<div class='nav-item--checkout-box__container__all-checkout-items--checkout-item col-lg-12'><div class='row align-items-center'><div class='col-3' style="padding: 0px"><div style="background-image: url('../media/`+element.img_url+`'); background-size: cover; background-position: center; height: 60px; width: 100%"></div></div><div class='col-9'><a href="/posts/`+element.post_id+`"><h6 class='checkout-item--name'>`+element.post_title+`</h6></a><p class='checkout-item--price'>Rs `+element.price+`</p></div></div></div>` 
                        );
                    });
                })
            }else{                
                $('.checkout-box__container').hide();
                $('.check-box__container--empty').show();
            }            
        }
    })
}
